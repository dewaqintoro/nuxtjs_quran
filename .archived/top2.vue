<template id="backToTop">
  <div>
    ...
    <transition name="back-to-top-fade">
      <div class="vue-back-to-top" @click="backToTop">
        <slot>
          <div class="default">
            <span>
              <div id="app">
              ...
              <button @click="backToTop">@click="backToTop"</button>
            </span>
          </div>
        </slot>
      </div> 
...
<script>
import { computed, ref, useAsync, useContext, onMounted , onUnmounted } from '@nuxtjs/composition-api'
export default {
  name: 'Quran',
  setup(_, {emit}){
    ...
    const right= '30px'
    const thisWidth = ref('')
    const thisHeight = ref('')

    if (process.browser){
      thisWidth.value = window.innerWidth
      thisHeight.value = window.innerHeight
      console.log('thisWidth',thisWidth);
      console.log('thisHeight', thisHeight);
    }

    if(thisWidth.value && thisHeight.value){
      console.log('ada windows')
      window.smoothscroll = () => {
    ...
    function catchScroll () {
      visible.value = (window.pageYOffset > parseInt(visibleoffset))
    }
